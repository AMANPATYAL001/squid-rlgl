...
def drawCircle():
    return random.randint(13,20),random.randint(50,200),random.randint(50,200)
        ...
        while True:
            ...
            if not splash:
                ...
                if start and not win and not kill:
                    j+=1
                if end and not kill:
                    cv2.rectangle(image,(20,20),(220,220),(0,255,0),2)
                ...
                if x and y and end and not kill:
                ...
                if 670-i>82:
                    ...
                    for k,j in zip(np.random.randint(0,1200,50),np.random.randint(0,720,50)):
                ...
                if end:
                    image[250:340,140:290]=boss

                    image[230:330,980:1043]=circle
                    image[570:670,1080:1143]=square
                    image[470:570,40:103]=triangle
                    image[5:80,585:695]=girl
                
                if j>=girl_movement[0] and j<=girl_movement[1] and start:
                    ...
                    if girl_movement[1]-j<20 and end:
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        for id,lm in enumerate(hand_landmarks.landmark):
                            if not start:
                                if id==12:
                                    cv2.rectangle(image,(int(lm.x*w)-15,int(lm.y*h)-15),(int(lm.x*w)-7+30,int(lm.y*h)-7+30),(random.randint(0,155),0,random.randint(0,255)),-1)
                                    if int(lm.x*w)<210 and int(lm.y*h)<210:
                                        start=True
                                if id==4:
                                    cv2.rectangle(image,(int(lm.x*w)-15,int(lm.y*h)-15),(int(lm.x*w)-7+30,int(lm.y*h)-7+30),(0,0,random.randint(0,255)),3)
                                if id==8:
                                    cv2.circle(image,(int(lm.x*w),int(lm.y*h)),20,(0,random.randint(0,255),random.randint(0,255)),3)
                                if id==16:
                                    cv2.circle(image,(int(lm.x*w),int(lm.y*h)),20,(random.randint(0,255),0,random.randint(0,255)),3)
                                if id==20:
                                    cv2.rectangle(image,(int(lm.x*w)-15,int(lm.y*h)-15),(int(lm.x*w)-7+30,int(lm.y*h)-7+30),(0,0,random.randint(0,255)),3)
